...
// =============================================================================
// OPSLAG-ADAPTER
// De tool gebruikt window.storage (een async key-value store) die alleen
// bestaat binnen de Claude-artifact-omgeving. In een gewone browser (bv.
// GitHub Pages) is window.storage undefined -> profielen opslaan crasht met
// "Cannot read properties of undefined (reading 'set')".
// Hieronder voorzien we, enkel als de echte storage ontbreekt, een variant met
// dezelfde API (get/set/delete/list) bovenop het standaard localStorage van de
// browser. Profielen, tracking, favorieten enz. worden zo lokaal op het toestel
// bewaard. We overschrijven nooit een bestaande window.storage.
// =============================================================================
if (typeof window !== 'undefined' && !window.storage) {
  const PREFIX = 'hb-store:';
  window.storage = {
    async get(key) {
      try {
      ...
      try {
        for (let i = 0; i < window.localStorage.length; i++) {
          ...
          if (full && full.startsWith(PREFIX)) {
            const appKey = full.slice(PREFIX.length);
            if (!prefix || appKey.startsWith(prefix)) keys.push(appKey);
          }
        }
      } catch (e) {}
      return { keys };
